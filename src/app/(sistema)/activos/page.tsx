...
/**
 * Listado de activos con búsqueda, paginación y acciones por fila.
 *
 * La data viene de `useActivos` (paginada) y `useUbicaciones` (para resolver
 * nombres). El debounce local evita pegar al backend con cada tecla.
 */
...
// Sub-componentes 
// Badge de color según el estado del activo (operativo, mantenimiento, etc.)
function EstadoBadge({ estado }: { estado: ActivoEstado }) {
  const cls = ESTADO_BADGE_STYLES[estado] ?? ESTADO_BADGE_STYLES['_fallback'];
  return (
    <span className={`inline-block px-4 py-1.5 rounded-full text-sm font-semibold ${cls}`}>
      {formatEstadoActivo(estado)}
    </span>
  );
}
...
export default function ActivosPage() {
  ...
  // Aplana el árbol de ubicaciones en un mapa id → nombre
  ...
  // Resuelve el nombre de ubicación por cada activo (o muestra el ID si no existe)
  ...
  // Debounce de búsqueda: espera 300ms antes de disparar la petición
  useEffect(() => {
  ...
  // Pide confirmación antes de borrar; muestra alert si falla
  ...
  return (
    <div className="flex-1 bg-[#F3F4F6] p-8 overflow-y-auto">
      <PageHeader
        title="Activos"
        subtitle="Equipos registrados en el inventario"
        variant="activos"
        searchValue={search}
