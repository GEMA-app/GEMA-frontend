...
export default function RegistrarActivoPage() {
  ...
  const validateForm = () => {
    ...
    if (!formData.ubicacion.trim()) {
      nextErrors.ubicacion = 'La ubicación física es obligatoria.';
    }
    if (!formData.responsable.trim()) {
      nextErrors.responsable = 'El responsable o custodio es obligatorio.';
    }
    if (!formData.fechaCompra) {
      nextErrors.fechaCompra = 'La fecha de compra es obligatoria.';
    }
    if (!formData.garantiaHasta) {
      nextErrors.garantiaHasta = 'La fecha de garantía es obligatoria.';
    }
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (Object.keys(nextErrors).length === 0) {
      ...
      // Aquí puedes agregar la llamada a la API o guardar en el estado global según sea necesario.
    } else {
          ...
          <form id="registrar-activo-form" onSubmit={handleSubmit} className="lg:col-span-2 space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-xs text-gray-500 mb-1" htmlFor="categoria">Categoría</label>
                  <input
                    id="categoria"
                    name="categoria"
                    type="text"
                    ...
                    onChange={handleInputChange}
                    className={`w-full bg-transparent border-b py-1.5 outline-none focus:border-[#E59D12] transition-colors text-sm ${errors.categoria ? 'border-red-500' : 'border-gray-400'}`}
                  />
            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  <label className="block text-xs text-gray-500 mb-1" htmlFor="responsable">Responsable / Custodio</label>
                  <input
                    id="responsable"
                    name="responsable"
                    type="text"
                    ...
                    onChange={handleInputChange}
                    className={`w-full bg-transparent border-b py-1.5 outline-none focus:border-[#E59D12] transition-colors text-sm ${errors.responsable ? 'border-red-500' : 'border-gray-400'}`}
                  />
                  {errors.responsable && <p className="text-xs text-red-600 mt-1">{errors.responsable}</p>}
                </div>
                <div>
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={handleInputChange}
                    style={{ WebkitAppearance: 'none', MozAppearance: 'textfield' }}
                    className={`date-input w-full bg-transparent border-b py-1.5 outline-none focus:border-[#E59D12] transition-colors text-sm ${errors.garantiaHasta ? 'border-red-500' : 'border-gray-400'}`}
                  />
                  {errors.garantiaHasta && <p className="text-xs text-red-600 mt-1">{errors.garantiaHasta}</p>}
                </div>
              </div>
            </div>
