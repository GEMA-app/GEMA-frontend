...
export default function EditarActivoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setTimeout(() => router.push(`/activos/ficha_de_activo?id=${activoId}`), 800);
    } catch (err) {
  ...
  return (
    <div className="flex-1 flex flex-col overflow-y-auto bg-white p-8 w-full font-sans">
      ...
      <div className="mb-6">
        <Link href={`/activos/ficha_de_activo?id=${activoId}`} className="flex items-center text-gray-700 hover:text-black font-medium transition-colors gap-2 w-fit">
      ...
      <div className="rounded-3xl p-8 border border-gray-100 shadow-sm flex flex-col gap-8" style={{ backgroundColor: 'rgba(46, 70, 101, 0.05)' }}>
        <div className="flex justify-between items-start border-b-2 border-[#2E4365]/20 pb-4">
          ...
          <div className="flex gap-3">
            <Link href={`/activos/ficha_de_activo?id=${activoId}`} className="px-6 py-2.5 bg-[#F3D58D] text-gray-900 font-semibold rounded-full text-sm shadow-sm hover:brightness-95 transition-all">
              Cancelar
            </Link>
            <button
              type="submit"
