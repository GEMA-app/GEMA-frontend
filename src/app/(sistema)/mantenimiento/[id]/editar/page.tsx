...
import { getOrdenTrabajoById, updateOrdenTrabajo } from '@/services/ordenes-trabajo';
import { formatEstadoOT, formatTipoMantenimiento } from '@/lib/orden-trabajo';
import type { TipoMantenimientoOT, OrdenTrabajo } from '@/types/orden-trabajo';

type Prioridad = 'baja' | 'media' | 'alta';

function EditarOrdenContent() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const o = await getOrdenTrabajoById(id);
        if (cancelled) return;
        setOrden(o);
        if (o) {
          setForm({
