...
import { Plus, Eye, Pencil, Trash, ChevronLeft, ChevronRight, ClipboardList } from 'lucide-react';
  ...
  return (
    <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
      ...
      <div className="flex items-center justify-between mb-6">
        ...
        <h3 className="text-lg font-bold text-gray-900 capitalize">{mes} {anio}</h3>
...
function OrdenesTrabajoPage() {
  ...
  const handleDelete = useCallback(
    async (id: string, codigo: string) => {
      if (!window.confirm(`¿Eliminar la orden "${codigo}"? Esta accion no se puede deshacer.`)) return;
      try {
  ...
  const emptyMessage = debouncedSearch
    ? 'No se encontraron ordenes para "' + debouncedSearch + '".'
  ...
  return (
    <div className="flex-1 bg-[#F3F4F6] p-8 overflow-y-auto">
      <PageHeader
        title="Mantenimiento"
        subtitle="Gestion de ordenes de trabajo y mantenimientos"
        variant="activos"
        searchValue={search}
      ...
      <div className="flex gap-2 mb-6">
        <button
          ...
          className={'px-4 py-2 rounded-xl text-sm font-semibold transition-colors ' + (vista === 'ordenes' ? 'bg-[#2B405B] text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50')}
        >
          Ordenes de trabajo
        </button>
        <button
          onClick={() => setVista('calendario')}
          className={'px-4 py-2 rounded-xl text-sm font-semibold transition-colors ' + (vista === 'calendario' ? 'bg-[#2B405B] text-white' : 'bg-white text-gray-600 border border-gray-200 hover:bg-gray-50')}
        >
          Calendario
        </button>
        <Link
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-8">
        ...
        <RequestState
          ...
          error={error}
          empty={empty}
          loadingMessage="Cargando ordenes de trabajo..."
        ...
        >
          ...
          {!loading && !empty && meta.lastPage > 1 && (
            <nav className="mt-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between text-sm text-gray-600" aria-label="Paginacion">
              <p>Pagina {meta.page} de {meta.lastPage} -- {meta.total} ordenes</p>
