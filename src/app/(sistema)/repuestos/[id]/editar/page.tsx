  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (err instanceof ApiError && err.status === 409) {
        setSubmitStatus('Conflicto de versión: el repuesto fue modificado por otro usuario. Recarga los datos e intenta de nuevo.');
      } else {
      ...
      <div className="flex-1 flex flex-col overflow-y-auto bg-white p-8 w-full font-sans">
        ...
        <div className="rounded-3xl p-8 border border-gray-100 shadow-sm flex flex-col gap-8" style={{ backgroundColor: 'rgba(46, 70, 101, 0.05)' }}>
          ...
          <form id="editar-repuesto-form" onSubmit={handleSubmit} className="space-y-8 max-w-2xl">
            ...
            {submitStatus && (
              <p className={`text-sm ${submitStatus.includes('Error') || submitStatus.includes('error') || submitStatus.includes('Conflicto') ? 'text-red-600' : 'text-emerald-700'}`}>{submitStatus}</p>
            )}
          </form>
        </div>
      </div>
